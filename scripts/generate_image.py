#!/usr/bin/env python3
"""Generate one image through the configured Midlight relay without exposing API credentials."""
...
DEFAULT_BASE_URL = "https://cc.midlight.top/v1"
DEFAULT_MODEL = "gpt-image-2"
MAX_IMAGE_BYTES = 50 * 1024 * 1024
BASE_URL_ENV_VARS = ("MIDLIGHT_API_BASE_URL", "CODER_API_BASE_URL")
CONFIG_ENV_VARS = ("MIDLIGHT_API_CONFIG_PATH", "CODER_API_CONFIG_PATH")
API_KEY_ENV_VARS = ("MIDLIGHT_API_KEY", "CODER_API_KEY")
...
def api_base_url() -> str:
    base_url = next(
        (os.environ[name].strip() for name in BASE_URL_ENV_VARS if os.environ.get(name, "").strip()),
        DEFAULT_BASE_URL,
    ).rstrip("/")
    parsed = urllib.parse.urlparse(base_url)
    if parsed.scheme not in {"https", "http"} or not parsed.netloc:
        raise SkillError("the configured API base URL must be an absolute http(s) URL")
    if parsed.scheme == "http" and parsed.hostname not in {"127.0.0.1", "localhost", "::1"}:
        raise SkillError("the configured API base URL must use HTTPS outside local testing")
    return base_url


def local_config_path() -> Path:
    configured_path = next(
        (os.environ[name].strip() for name in CONFIG_ENV_VARS if os.environ.get(name, "").strip()),
        "",
    )
    if configured_path:
        return Path(configured_path).expanduser()
    config_home = Path(os.environ.get("XDG_CONFIG_HOME", Path.home() / ".config"))
    return config_home / "midlight-image" / "credentials.json"


def environment_api_key() -> str:
    return next(
        (os.environ[name].strip() for name in API_KEY_ENV_VARS if os.environ.get(name, "").strip()),
        "",
    )
...
def write_local_api_key(config_path: Path, api_key: str) -> None:
    ...
    set_private_permissions(config_path.parent, 0o700)
...
def create_workflow_state(prompt: str, input_image: str | None = None) -> Path:
    ...
    key_available = bool(environment_api_key() or read_local_api_key(local_config_path()))
    state_directory = Path(tempfile.mkdtemp(prefix="midlight-image-"))
    set_private_permissions(state_directory, 0o700)
...
def remove_workflow_state(state_path: Path) -> None:
    ...
    if state_path.parent.parent == temporary_root and state_path.parent.name.startswith("midlight-image-"):
...
def read_api_key() -> str:
    ...
    if api_key:
        return api_key
    raise SkillError("no API key found; run --configure or set MIDLIGHT_API_KEY")
...
def build_edit_multipart(payload: dict[str, Any], image_path: Path) -> tuple[bytes, str]:
    ...
    if mime_type not in {"image/png", "image/jpeg", "image/webp"}:
        raise SkillError("--image must be a PNG, JPEG, or WebP file")
    boundary = f"----MidlightRelayImageEdit{secrets.token_hex(16)}"
    body = bytearray()
